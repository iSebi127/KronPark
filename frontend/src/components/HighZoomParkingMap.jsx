  ...
  zones.forEach(({ key, rows, cols, startY, startX }) => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        spots.push({ id: `${key}${counter}`, spotNumber: `${key}${counter}`, status: 'free', bounds: [[y1, x1], [y2, x2]] });
        counter += 1;
      }
    }
  });
...
const statusToOptions = (status, isSelected) => {
  if (isSelected) return { color: '#0ea5e9', weight: 3, fillColor: '#38bdf8', fillOpacity: 0.85, opacity: 1 };
...
function isValidBounds(bounds) {
  ...
  const y1 = Number(a[0]), x1 = Number(a[1]), y2 = Number(b[0]), x2 = Number(b[1]);
...
const HighZoomParkingMap = ({ layout, onReserve }) => {
  ...
  if (allCoords.length) {
    ...
    if ([minY, maxY, minX, maxX].every(Number.isFinite)) {
      boundary = [[minY - 40, minX - 40], [maxY + 40, maxX + 40]];
    }
  }
    ...
    <div className="w-full" data-cy="lot-layout">
      <div className="flex items-center gap-4 mb-3">
        <div className="flex items-center gap-2 bg-slate-800 rounded-lg px-3 py-1.5">
          ...
          <span className="text-sm text-slate-300">Libere: <span className="text-white font-bold">{freeCount}</span></span>
        </div>
        <div className="flex items-center gap-2 bg-slate-800 rounded-lg px-3 py-1.5">
          <div className="w-3 h-3 rounded-sm bg-red-500"></div>
          <span className="text-sm text-slate-300">Ocupate: <span className="text-white font-bold">{occupiedCount}</span></span>
        </div>
        <div className="flex items-center gap-2 bg-slate-800 rounded-lg px-3 py-1.5">
